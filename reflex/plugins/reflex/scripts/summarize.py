...
Supports Ollama (default), Brainbox proxy, OpenAI, and Anthropic.

Usage:
    # Ollama (default outside containers, no extra deps)
    python summarize.py transcript.vtt

    # Brainbox proxy (auto-detected inside containers when BRAINBOX_HUB_URL is set)
    python summarize.py transcript.vtt --llm brainbox
...
Environment variables (REFLEX_TRANSCRIPT_ prefix):
    REFLEX_TRANSCRIPT_LLM     LLM provider (default: brainbox if BRAINBOX_HUB_URL set, else ollama)
    REFLEX_TRANSCRIPT_MODEL   Model name override
    BRAINBOX_HUB_URL          Brainbox API URL (auto-injected in containers)
    BRAINBOX_API_KEY           Brainbox API key (or BRAINBOX_API_KEY_FILE for file path)
"""
...
class BrainboxProvider:
    """Proxy through brainbox API's Ollama endpoint (zero external deps)."""

    def __init__(self, model: str, hub_url: str, api_key: str = ""):
        self.model = model
        self.hub_url = hub_url.rstrip("/")
        self.api_key = api_key

    def complete(self, system: str, user: str) -> str:
        url = f"{self.hub_url}/api/ollama/chat"
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": system},
        ...
        try:
            ...
            raise LLMConnectionError(
                f"Could not connect to brainbox at {self.hub_url}. "
                f"Is brainbox running? Error: {e}"
            )
        except json.JSONDecodeError as e:
            raise LLMResponseError(f"Invalid JSON from brainbox: {e}")

        try:
            return body["message"]["content"]
        except (KeyError, TypeError):
            raise LLMResponseError(f"Unexpected brainbox response structure: {body}")
...
def create_provider(name: str, model: str | None = None) -> LLMProvider:
    ...
    if name == "ollama":
        ...
        hub_url = os.environ.get("BRAINBOX_HUB_URL", "http://localhost:9999")
        api_key = os.environ.get("BRAINBOX_API_KEY", "")
        if not api_key:
            key_file = os.environ.get("BRAINBOX_API_KEY_FILE", "")
            if key_file and Path(key_file).exists():
                api_key = Path(key_file).read_text().strip()
        return BrainboxProvider(model=resolved_model, hub_url=hub_url, api_key=api_key)
    else:
        raise SummarizeError(
            f"Unknown provider '{name}'. Supported: ollama, openai, anthropic, brainbox"
        )
...
examples:
  ...
  python summarize.py meeting.docx --output summary.md
    ...
    parser.add_argument(
        "--llm",
        choices=["ollama", "openai", "anthropic", "brainbox"],
        default=None,
        help="LLM provider (default: 'brainbox' when BRAINBOX_HUB_URL is set, else 'ollama')",
    )
    parser.add_argument(
        "--model",
        default=None,
    ...
    # Resolve LLM provider: flag > env > auto-detect > default
    # Auto-detect: prefer brainbox when BRAINBOX_HUB_URL is set (inside a container)
    default_llm = "brainbox" if os.environ.get("BRAINBOX_HUB_URL") else "ollama"
    llm_name = args.llm or os.environ.get("REFLEX_TRANSCRIPT_LLM", default_llm)
