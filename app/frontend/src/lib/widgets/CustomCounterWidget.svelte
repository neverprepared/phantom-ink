<script lang="ts">
  ...
  let { config }: { config: CustomCounterConfig } = $props();
  ...
  async function fetch() {
    ...
    if (!a) return;
    try {
      count = await a.GetWidgetCount(config.api, JSON.stringify(config.filter ?? {}));
      error = false;
    } catch {
      error = true;
    } finally {
...
<div class="stat-card">
  ...
  {/if}
  <span class="stat-sub">{config.api.replace('_', ' ')}
    {#if config.filter?.status}· {config.filter.status}{/if}
  </span>
</div>
