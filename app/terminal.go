...
// OpenLocalSession opens a new terminal tab with an interactive claude session
// in workDir. Tries iTerm2 first, falls back to Terminal.app.
func (a *App) OpenLocalSession(workDir string) error {
	// Validate workDir to prevent AppleScript injection — only allow printable
	// ASCII excluding double-quote and backslash.
	for _, c := range workDir {
		if c == '"' || c == '\\' || c < 0x20 || c > 0x7e {
			return fmt.Errorf("invalid character in workDir: %q", workDir)
		}
	}
	if workDir == "" {
		workDir = "~"
...
return "not_found"`, workDir)
	...
	return fmt.Errorf("could not open terminal: no iTerm2 or Terminal.app found")
}
