...
Secrets are organized in vaults with scoped access. Each agent workload gets its own vault so permissions stay narrow.
...
The `.envrc` file contains only `op://` references. It is safe to commit.
...
1Password holds wrapped (encrypted) DEKs, not plaintext secrets. A separate Cloud KMS holds the KEK.
...
| **Cloud KMS** | Key Encryption Key (KEK) | Attacker can unwrap DEKs — but needs 1Password access too |
...
| **CI / automation** | OIDC federation — CI platform identity token, no static Service Account token |
| **CI fallback** | Service Account token: pinned to IP ranges, auto-rotated, anomaly-monitored |
...
| Feature | Phase |
|---|---|
| Break-glass procedure for 1Password/KMS outages | PHASE_3 |
