...
def _request_as_session(
    ...
    Falls back to the API key if no token is available. This is how a session
    tells the server WHICH participant it is — the conversation tools below
    depend on it.
    """
    ...
    url = f"{_api_url()}{path}"
    # A GET carrying a request body is malformed and some proxies drop it.
