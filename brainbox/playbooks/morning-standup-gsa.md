...
Replace `YOUR_ORG` in `JIRA_URL` below once the gsa Jira instance is configured.

- [ ] Use the Jira MCP tool to fetch all In Progress tickets assigned to me across all projects at https://YOUR_ORG.atlassian.net — collect Key, Summary, Status, and the date of the most recent comment or status change for each ticket; flag any ticket with no update in the last 48 hours as stale
